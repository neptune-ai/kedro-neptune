...
def prepare_dataset(planets: pd.DataFrame):
    ...
    return TabularDataLoaders.from_df(
        ...
        cont_names=["Mass (1024kg)", "Diameter (km)"],
        cat_names=["Has Many Moons"],
...
class Objective(object):
    def __init__(self, neptune_run: neptune.Run, model: fastai.tabular.model.TabularModel):
        self.neptune_run = neptune_run
        self.model = model
        self.test_dataset = pd.DataFrame(
            {"Mass (1024kg)": [1000, 1000000], "Diameter (km)": [40, 7000], "Has Many Moons": [False, True]},
            index=range(2),
        )
        ...
        self.model.fit_one_cycle(
            ...
            **params,
            cbs=[
                SaveModelCallback(),
                NeptuneCallback(
                    run=self.neptune_run, base_namespace=f"kedro/moons_classifier/trials/trials/{trial._trial_id}"
                ),
            ],
        )
...
def optimize(neptune_run: neptune.handler.Handler, model: fastai.tabular.model.TabularModel):
    ...
    study.optimize(
        Objective(neptune_run=neptune_run.container, model=model),
        n_trials=5,
        callbacks=[optuna_utils.NeptuneCallback(run=neptune_run.container, base_namespace="kedro/moons_classifier")],
    )
    neptune_run.container.wait()
